...
Demand response optimization becomes the central component of the virtual power plant. In order to minimize the energy waste present in the network, the problem is reduced to a combinatorial optimization problem; more specifically, it can be viewed as a more sophisticated and ample assignment problem. Given the demands for specific time slots, the consumers respectively; together with the producers that are available to satisfy those demands, the optimization consists in assigning the producers to the consumers so that the energy waste in the system is minimal while maximizing the satisfaction of the overall network members. This assignment considers also the price constraint, the consumer specifying the maximal price it’s willing to accept per kilowatt, while the producer the minimal price it’s willing to sell for, the final price decided being the average, simulating therefore the self-adjusting behavior of a free market. 
This constrained assignment optimization problem described above is approached as a mixed integer programming problem. The solver that is considered for this feat is one of the fastest academically developed mixed integer programming solvers, namely SCIP(https://link.springer.com/article/10.1007/s12532-008-0001-1), the algorithm being adapted/extended in order to optimize the problem in case successfully.
